...
chromadb_path = os.path.join('chromadb', 'KB_DB.db')
...
def show_all_tables():
    """Display all table names in the database in the sidebar."""
    # Connect to the SQLite database
    conn = sqlite3.connect(chromadb_path)
    ...
    # Close the database connection
    conn.close()
    # Display table names in the sidebar
    st.write("All Tables in Database")
    for table in tables:
        st.write(table[0])
...
def show_all_entries():
    ...
    if not entries.empty:
        ...
        for _, row in entries.iterrows():
            st.sidebar.write(f"**ID:** {row['id']}, **Title:** {row['Title']}, **Content:** {row['content']}")
...
st.sidebar.button("Show All Tables", on_click=show_all_tables)
